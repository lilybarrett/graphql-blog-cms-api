...
// the above two are imported so we can create a scalar DateTime type
const { User, Post, Tag } = require('../models');
// models, duh
const bcrypt = require('bcrypt');
// used for hashing a user's password
const jwt = require('jsonwebtoken');
// used to create a JSON web token to authenticate users
const slugify = require('slugify');
// used to create slugs
require('dotenv').config();
// used to allow us to read from our .env file
...
const resolvers = {
    ...
    DateTime: new GraphQLScalarType({
        ...
        parseValue(value) {
            // value from the client is converted to a Date object
            return new Date(value);
        },
        serialize(value) {
            const date = new Date(value);
            // a date in ISO format is returned to the client
